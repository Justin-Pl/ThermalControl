...
extern void temp_graph_draw_custom(Clay_BoundingBox box, void* context);
...
static inline Clay_Dimensions Raylib_MeasureText(Clay_StringSlice text, Clay_TextElementConfig *config, void *userData) {
    ...
    while (i < text.length)
    {
        ...
        int codepoint = GetCodepointNext(&text.chars[i], &codepointByteCount);

        int index = GetGlyphIndex(fontToUse, codepoint);
        if (fontToUse.glyphs[index].advanceX != 0) lineTextWidth += fontToUse.glyphs[index].advanceX;
        else lineTextWidth += (fontToUse.recs[index].width + fontToUse.glyphs[index].offsetX);

        i += (codepointByteCount > 0) ? codepointByteCount : 1;
        lineCharCount++;
    }
    ...
    for (int j = 0; j < renderCommands.length; j++)
    {
        ...
        switch (renderCommand->commandType)
        {
            ...
            case CLAY_RENDER_COMMAND_TYPE_RECTANGLE: {
                ...
                Color bgColor = CLAY_COLOR_TO_RAYLIB_COLOR(config->backgroundColor);
                float tl = config->cornerRadius.topLeft;
                float tr = config->cornerRadius.topRight;
                float bl = config->cornerRadius.bottomLeft;
                float br = config->cornerRadius.bottomRight;

                if (tl <= 0 && tr <= 0 && bl <= 0 && br <= 0) {
                    // Kein Radius irgendwo - einfaches Rechteck, wie bisher.
                    DrawRectangle(boundingBox.x, boundingBox.y, boundingBox.width, boundingBox.height, bgColor);
                }
                else {
                    // Allgemeine Loesung fuer vier UNABHAENGIGE Eckradien.
                    // DrawRectangleRounded kennt nur EINEN Radius fuers ganze
                    // Rechteck - deshalb bauen wir die Form selbst aus
                    // Rechtecken (fuer die geraden Kanten) + Kreissektoren
                    // (fuer die vier Ecken) zusammen.
                    float x = boundingBox.x, y = boundingBox.y, w = boundingBox.width, h = boundingBox.height;
                    float topStripH = (tl > tr) ? tl : tr;
                    float bottomStripH = (bl > br) ? bl : br;

                    // Mittlerer Bereich (volle Breite, zwischen oberem und unterem Streifen)
                    if (h - topStripH - bottomStripH > 0) {
                        DrawRectangle(x, y + topStripH, w, h - topStripH - bottomStripH, bgColor);
                    }
                    // Oberer Streifen (zwischen den beiden oberen Ecken)
                    if (topStripH > 0 && w - tl - tr > 0) {
                        DrawRectangle(x + tl, y, w - tl - tr, topStripH, bgColor);
                    }
                    // Unterer Streifen (zwischen den beiden unteren Ecken)
                    if (bottomStripH > 0 && w - bl - br > 0) {
                        DrawRectangle(x + bl, y + h - bottomStripH, w - bl - br, bottomStripH, bgColor);
                    }
                    // Fuellstuecke fuer den Fall, dass zwei Ecken auf derselben
                    // Kante UNTERSCHIEDLICHE Radien haben (z.B. topLeft=8,
                    // topRight=0) - ohne das entstuende hier eine Luecke.
                    if (tr > tl) {
                        DrawRectangle(x, y + tl, tl, topStripH - tl, bgColor);
                    }
                    else if (tl > tr) {
                        DrawRectangle(x + w - tr, y + tr, tr, topStripH - tr, bgColor);
                    }
                    if (br > bl) {
                        DrawRectangle(x, y + h - bottomStripH, bl, bottomStripH - bl, bgColor);
                    }
                    else if (bl > br) {
                        DrawRectangle(x + w - br, y + h - bottomStripH, br, bottomStripH - br, bgColor);
                    }

                    // Vier Eckabrundungen, jede mit ihrem EIGENEN Radius.
                    // Winkel: 0=Ost, 90=Sued, 180=West, 270=Nord (im Uhrzeigersinn,
                    // da Y nach unten waechst).
                    if (tl > 0) DrawCircleSector((Vector2) { x + tl, y + tl }, tl, 180, 270, 8, bgColor);
                    if (tr > 0) DrawCircleSector((Vector2) { x + w - tr, y + tr }, tr, 270, 360, 8, bgColor);
                    if (br > 0) DrawCircleSector((Vector2) { x + w - br, y + h - br }, br, 0, 90, 8, bgColor);
                    if (bl > 0) DrawCircleSector((Vector2) { x + bl, y + h - bl }, bl, 90, 180, 8, bgColor);
                }
                break;
            }
            ...
            case CLAY_RENDER_COMMAND_TYPE_CUSTOM: {
                ...
                switch (customElement->type) {
                    ...
                    case CUSTOM_LAYOUT_ELEMENT_TYPE_GRAPH: {
                        temp_graph_draw_custom(renderCommand->boundingBox, customElement->customData.graph.graph_context);
                        break;
                    }
                    default: break;
                }
